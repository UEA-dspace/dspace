...
<link rel="stylesheet" href="<%= request.getContextPath() %>/static/css/bootstrap-treeview.css" type="text/css" />
<%--<link rel="stylesheet" href="<%= request.getContextPath() %>/static/css/bootstrap-treeview.min.css" type="text/css" />--%>
<script type="text/javascript" src="<%= request.getContextPath() %>/static/js/bootstrap-treeview.js" ></script>
...
<dspace:layout titlekey="jsp.community-list.title">
    ...
    <%
        ...
        for (int i = 0; i < communities.size(); i++){
            ...
            if(i>0) out.println(",");
            if(i==0) out.println("{text: '" + c.getName() + "', href: '/handle/" + c.getHandle()  + "', tags:['"+ic.getCount(c)+"'], state: {expanded:true}, nodes:[");
            else out.println("{text: '" + c.getName() + "', href: '/handle/" + c.getHandle()  + "', tags:['"+ic.getCount(c)+"'], state: {expanded:false}, nodes:[");

            List<Collection> cols = (List<Collection>) collectionMap.get(c.getID().toString());
//            Collection[] cols = (Collection[]) collectionMap.get(c.getID().toString());
            if (cols != null && cols.size() > 0)
            {
                for (int j = 0; j < cols.size(); j++)
                {
                    ...
                    out.println("\t{text: '" + cols.get(j).getName() + "', href: '/handle/" + cols.get(j).getHandle()  + "', tags:['"+ic.getCount(cols.get(j))+"']}");
